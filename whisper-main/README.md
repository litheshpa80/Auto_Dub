...
The following command will transcribe speech in audio or video files, using the `turbo` model:


```bash
whisper audio.flac audio.mp3 audio.wav video.mp4 video.mkv --model turbo
```

You can now pass video files (such as `.mp4`, `.mkv`, `.avi`, etc.) directly; Whisper will automatically extract the audio track for transcription.
...
# You can also transcribe video files directly:
result = model.transcribe("video.mp4")
print(result["text"])
```


Internally, the `transcribe()` method reads the entire file (audio or video) and processes the audio with a sliding 30-second window, performing autoregressive sequence-to-sequence predictions on each window.
